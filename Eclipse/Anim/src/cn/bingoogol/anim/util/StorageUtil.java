package cn.bingoogol.anim.util;

import java.io.File;

import cn.bingoogol.anim.App;
...
public class StorageUtil {
	...
	public static String getRealPathByUri(Uri uri) {
		if ("content".equalsIgnoreCase(uri.getScheme())) {
			...
			Cursor cursor = null;
			try {
				cursor = App.getInstance().getContentResolver().query(uri, projection, null, null, null);
	...
	public static long getAvailMemSize() {
		...
		((ActivityManager) App.getInstance().getSystemService(Context.ACTIVITY_SERVICE)).getMemoryInfo(memoryInfo);
